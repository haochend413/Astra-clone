...
// Float kernel v6
__global__ __launch_bounds__(256,12)
void merge_state_kernel_v6_float(
...
// Half kernel v6
__global__ __launch_bounds__(256,12)
void merge_state_kernel_v6_half(
...
// BFloat16 kernel v6
__global__ __launch_bounds__(256,12)
void merge_state_kernel_v6_bf16(
...
void merge_state(at::Tensor v_a, at::Tensor s_a,
  ...
  uint32_t seq_len   = (uint32_t)v_a.size(0);
  uint32_t num_heads = (uint32_t)v_a.size(1);
  uint32_t head_dim  = (uint32_t)v_a.size(2);

  c10::cuda::OptionalCUDAGuard guard(v_a.device());
  auto stream = at::cuda::getCurrentCUDAStream();

  dim3 blocks(seq_len, num_heads);
  uint32_t threads = std::min(MAX_THREADS, std::max(NEXT_MULTIPLE(head_dim, 32u), 128u));
  uint32_t shared_mem = 2 * sizeof(float);

  bool ok = false;
  if (v_a.scalar_type() == at::kFloat) {
    merge_state_kernel_v6_float<<<blocks, threads, shared_mem, stream>>>(
      v_a.data_ptr<float>(), s_a.data_ptr<float>(),
      v_b.data_ptr<float>(), s_b.data_ptr<float>(),
      v_out.data_ptr<float>(), s_out.data_ptr<float>(), head_dim);
    ok = true;
  } else if (v_a.scalar_type() == at::kHalf) {
    merge_state_kernel_v6_half<<<blocks, threads, shared_mem, stream>>>(
      ...
      reinterpret_cast<__half*>(v_out.data_ptr()), s_out.data_ptr<float>(), head_dim);
    ok = true;
  } else if (v_a.scalar_type() == at::kBFloat16) {
    merge_state_kernel_v6_bf16<<<blocks, threads, shared_mem, stream>>>(
      ...
      reinterpret_cast<__nv_bfloat16*>(v_out.data_ptr()), s_out.data_ptr<float>(), head_dim);
    ok = true;
  }

  auto err = cudaGetLastError();
  TORCH_CHECK(ok && err == cudaSuccess, "merge_state kernel launch v6 failed");
}
