...
// Float kernel
__global__ __launch_bounds__(MAX_THREADS) void merge_state_kernel_v2_float(
...
// Half kernel
__global__ __launch_bounds__(MAX_THREADS) void merge_state_kernel_v2_half(
...
// BFloat16 kernel (scalar only)
__global__ __launch_bounds__(MAX_THREADS) void merge_state_kernel_v2_bf16(
...
void merge_state(at::Tensor v_a, at::Tensor s_a,
  ...
  uint32_t threads = std::min(head_dim, MAX_THREADS);

  bool ok = false;
  if (v_a.scalar_type() == at::kFloat) {
    merge_state_kernel_v2_float<<<blocks, threads, 0, stream>>>(
      v_a.data_ptr<float>(),
      s_a.data_ptr<float>(),
      v_b.data_ptr<float>(),
      s_b.data_ptr<float>(),
    ...
    merge_state_kernel_v2_half<<<blocks, threads, 0, stream>>>(
    ...
    merge_state_kernel_v2_bf16<<<blocks, threads, 0, stream>>>(
  ...
  TORCH_CHECK(ok && err == cudaSuccess, "merge_state kernel launch v2 failed");
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("merge_state", &merge_state, "merge_state (v2)");
}